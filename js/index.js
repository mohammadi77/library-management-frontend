...
function borrowBook(bookid) {
  ...
  http("https://karyar-library-management-system.liara.run/api/loans", {
    ...
    .then((response) => {
      ...
      message_alert = "کتاب با موفقیت به لیست اضافه شد ";
      alert.innerHTML = `
       <div class="massage">
        <p>${message_alert}</p>
      </div>
...
function returnBook(loanId) {
  http(
    ...
    .then((response) => {
      const alert = document.createElement("div");
      alert.classList.add("alert");
      message_alert = "کتاب  به کتابخانه داده شد";
      alert.innerHTML = `
       <div class="massage">
        <p>${message_alert}</p>
      </div>
...
function search_click() {
  document.getElementById("submit_search").addEventListener("click", (e) => {
    const value_search = document.getElementById("text-search").value.trim();
    const available = document.getElementById("available_search").checked;
    const unavailable = document.getElementById("unavailable_search").checked;
    search(value_search, available, unavailable);
  });
}
function search(value_search, available, unavailable) {
  const cachedBooks = cache.get("books");
  ...
  if (value_search) {
    filtered = filtered.filter((book) =>
      book.title.toLowerCase().includes(value_search.toLowerCase())
    );
  }
